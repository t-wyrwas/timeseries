...
@router.post("/{bucket_name}/{timeserie_name}", status_code=status.HTTP_201_CREATED)
def add_bucket(bucket_name: str, timeserie_name: str, timeserie = list[Point]):
    uow = SqlAlchemyUnitOfWork(db_config=DbConfig.from_env())
    with uow:
        repo = uow.get_bucket_repository()
        bucket = repo.get_by_name(bucket_name)
        if bucket is None:
            return PlainTextResponse(status_code=status.HTTP_400_BAD_REQUEST, content="Bucket does not exist")
        ts = bucket.get_by(name=timeserie_name)
        if ts is None:
            return PlainTextResponse(status_code=status.HTTP_400_BAD_REQUEST, content="Timeserie does not exist")
